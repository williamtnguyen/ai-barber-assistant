...
export function ChatPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      content: "Hello! I'm your AI haircut consultant. I can help you discover new styles, answer questions about hair, suggest suitable styles, and learn about your preferences. What would you like to do?",
      sender: 'bot',
      timestamp: new Date(),
    },
  ]);
  ...
  const [lastPhotoData, setLastPhotoData] = useState<string | null>(null);

  const handlePhotoCapture = (photoData: string, photoKey: string) => {
    setLastPhotoData(photoData);
    setInputMessage(`Just took the photo! The filepath is "${photoKey}". Can you please tell me my face shape?`);
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Use streaming API
      const streamGenerator = apiService.chatStream(userMessage.content);
      let fullResponse = '';
      ...
      <Card className="flex flex-row flex-1 overflow-hidden gap-0">
        <div className="flex-1 flex flex-col overflow-hidden border-r pt-4 w-3/4">
          ...
          <CardContent className="flex-1 flex flex-col overflow-hidden">
            ...
            {/* Input Area */}
            {!showWebcam && <div className="border-t pt-4">
              <Button variant="outline" className="mb-5" onClick={() => setShowWebcam(true)}>
                📸 Open Camera
              </Button>
        ...
        <CardContent className="p-4 w-1/4">
          ...
          <div className="flex flex-col gap-3.5">
            {[
              "What is a suitable hairstyle for me?",
              "What are nice low maintenance hairstyles?",
              "What hairstyles are well suited for my chiseled jawline?",
              "What hairstyles are well suited for my receding hairline?",
              "What hairstyles can I rock if I am bald?",
